...
class ClinicalDataGenerator:
    ...
    def create_clinical_patterns(self):
        ...
        for i in range(self.n_patterns):
            pattern = {
                ...
                'note_templates': [
                    f"Pattern {i} with severity level {{}}"
                ],
                # Add progression parameters
                'progression_rate': np.random.uniform(0.1, 0.5),  # How fast condition progresses
                'intervention_effectiveness': np.random.uniform(0.5, 0.9),  # How well interventions work
                'baseline_return_time': np.random.randint(1, 6)  # Typical months between visits
            }
            self.patterns.append(pattern)
            
    def generate_sample(self, pattern, severity) -> Tuple[np.ndarray, np.ndarray, np.ndarray, str]:
        ...
        return clinical_state, interventions, diagnoses, note_lines
    ...
    def generate_patient_trajectory(self, patient_id: int) -> PatientTrajectory:
        ...
        for visit_id in range(n_visits):
            ...
            clinical_state, interventions, diagnoses, notes = self.generate_sample(
                pattern, 
                current_severity
            )
    ...
    def generate_dataset(self, n_patients: int) -> List[PatientTrajectory]:
        ...
        return [
            self.generate_patient_trajectory(i) 
